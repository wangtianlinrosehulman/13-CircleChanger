...
class CircleChanger(object):
    ...
    def swallow(self, other_circle_changer):
        ...
        radius = 0.5 * rg.Point.get_distance_from(self.circle.center, other_circle_changer.circle.center)
        colors = self.colors + other_circle_changer.colors
        new = CircleChanger(point.x, point.y, radius, 'red', colors)
        return new
    ...
    def change_to_original_color(self):
        ...
        #   (below).  Third, implement and test this method.
        ################################################################
        self.circle.fill_color = self.origin_color
    ...
    def change_to_next_color_in_tuple(self):
        ...
        ################################################################
        self.count += 1
        k = self.count % len(self.colors) - 1
        if k >=0 :
